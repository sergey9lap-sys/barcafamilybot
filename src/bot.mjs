...
export function botHandlers(store,tg,cfg) {
 const mediaFiles=new Map(),coverContext=new AsyncLocalStorage();
 ...
 async function screen(chat,messageId,text,keyboard=[],photoPath=coverContext.getStore()||'/covers/home.png') {
   ...
   if(!/^\/(?:covers\/(?:home|mine|results|season|history|admin)\.png|portraits\/[\w-]+\.(?:jpg|png)|bot-banner(?:-v2)?\.png)$/.test(photoPath))photoPath='/bot-banner.png';
 ...
 async function handleUpdate(update,{acknowledged=false}={}) {
   ...
   if(callback){if(!acknowledged)void acknowledgeCallback(tg,callback);const [action,id,arg,score,version]=(callback.data||'').split('|');const mid=msg.message_id;
     try{
       ...
       if(action==='pick'){const d=await store.detail(id,user);if(d.match.state!=='open')return results(chat,user,mid,id);return screen(chat,mid,'<b>Чью оценку изменить?</b>',[...d.players.map((p,i)=>[button(`${p.name} · ${d.ballot.scores[p.id]||'—'}`,`p|${id}|${i}`)]),[button('Мои оценки',`mine|${id}`)]],'/covers/mine.png');}
       ...
       if(action==='season'){const r=await store.season(id),c=await store.content();return screen(chat,mid,`<b>Сезон ${esc(id)} · средняя оценка</b>\n\n${r.matches?`Команда: <b>${rating(r.team)} / 5</b> · ${r.matches} матчей\n\n${r.players.slice(0,20).map(p=>`${esc(p.name)} · ${rating(p.rating)} (${p.matches})`).join('\n')}`:'После первого закрытого настоящего голосования здесь появится сезонный рейтинг. Тестовые матчи не учитываются.'}\n\n${richHtml(c.season_hint)}`,[[button('В начало','home')]],'/covers/season.png');}
       if(action==='matches'){const ms=await store.matches();return screen(chat,mid,'<b>Выбери матч</b>',ms.slice(0,30).map(m=>[button(`${m.opponent} ${m.score} · ${m.played_on}`,`home|${m.id}`)]),'/covers/history.png');}
 ...
 function handle(update,options){const action=update.callback_query?.data?.split('|')[0];const publicActions=['home','p','v','mine','pick','done','r','season','matches'];const cover=action?(publicActions.includes(action)?'/covers/home.png':'/covers/admin.png'):(update.message?.text?.startsWith('/start')?'/covers/home.png':'/covers/admin.png');return coverContext.run(cover,()=>handleUpdate(update,options));}
